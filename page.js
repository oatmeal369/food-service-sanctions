  ...
  return (
    <>
      ...
      <main id="home">
        ...
        {!loading && !error && (
          <>
            <section className="dataset-overview" aria-label="전체 결과 현황">
              <p>
                전체 결과: <strong>{totalCount.toLocaleString()}건</strong>
              </p>
              <span>API에서 받은 {analyzedRows.length.toLocaleString()}건을 검색·분석합니다.</span>
            </section>
            ...
            <section className="summary-grid" aria-label="홈 요약">
              ...
              <SummaryCard
                title="현재 표시 중인 건수"
                value={`${visibleRows.length.toLocaleString()}건`}
                note={`필터 결과 ${filteredRows.length.toLocaleString()}건 중 표시`}
              />
              <SummaryCard
                title="최신 처분일"
